...
function validateAndUpdateUI(sessions) {
    ...
    const testStatus = getTestCompletionStatus(sessions);
    console.log('Test completion status:', testStatus);
    
    // Check if any test has incomplete channels - only for current session validation
...
function updateTrendAnalysisButton(hasIncompleteTests, incompleteTestsInfo) {
    ...
    if (trendButton) {
        if (hasIncompleteTests) {
            ...
            trendButton.style.opacity = '0.5';
            trendButton.title = `Trend analysis disabled: Missing channels for ${incompleteTestsInfo.length} test(s). Complete all required pathogen channels first.`;
            trendButton.innerHTML = 'View Trends (Disabled)';
        } else {
            trendButton.disabled = false;
            trendButton.style.opacity = '1';
            trendButton.title = 'View trend analysis';
...
function displayChannelCompletionStatus(testStatus) {
    const statusContainer = document.getElementById('channelCompletionStatus');
    if (!statusContainer) {
        console.log('Channel completion status container not found');
        return;
    }
...
async function updatePathogenChannelStatusInBreakdown() {
    try {
        ...
        if (data.sessions && data.sessions.length > 0) {
            const testStatus = getTestCompletionStatus(data.sessions);
...
function displayPathogenChannelStatusInBreakdown(testStatus) {
    ...
    // Get the most recent experiment pattern to filter by current work session
    let mostRecentExperiment = null;
    let mostRecentTimestamp = 0;
    
    Object.values(testStatus).forEach(test => {
        test.experiments.forEach(experiment => {
            // Find the most recent session for this experiment
            const maxSessionId = Math.max(...experiment.sessionIds);
            if (maxSessionId > mostRecentTimestamp) {
                mostRecentTimestamp = maxSessionId;
                mostRecentExperiment = experiment.experimentPattern;
            }
        });
    });
    ...
    Object.values(testStatus).forEach(test => {
        ...
        test.experiments.forEach(experiment => {
            // Only show the most recent experiment to avoid mixing multiple users' work
            if (!experiment.validation.isComplete && experiment.experimentPattern === mostRecentExperiment) {
                statusHtml += `
...
function groupSessionsByExperiment(sessions) {
    ...
    Object.entries(experimentGroups).forEach(([experimentPattern, groupSessions]) => {
        ...
        if (validSessions.length === 0) {
            ...
            // Single valid session - keep as is but clean up the filename
            const session = validSessions[0];
            session.filename = experimentPattern; // Clean display name
...
function calculatePositiveRate(session) {
    ...
    sortedFluorophores.forEach(fluorophore => {
        ...
        // Get pathogen target for display
        let pathogenTarget = fluorophore;
        if (typeof getPathogenTarget === 'function') {
            pathogenTarget = getPathogenTarget('BVAB', fluorophore) || fluorophore;
        }
    ...
    if (pathogenRates.length === 0 && session.well_results && session.well_results.length > 1000) {
        ...
        // Create rates from well_id extraction
            ...
            <tbody>
                ${sortedSessions.map(session => `
                    <tr data-session-id="${session.id}" class="history-row">
                        <td><strong><a href="javascript:void(0)" onclick="loadSessionDetails('${session.id}')" class="session-link">${session.filename}</a></strong></td>
                        ...
                        <td>${calculatePositiveRate(session)}${session.is_combined && session.session_ids && session.session_ids.length < 3 ? '<br><small style="color: #e67e22;">⚠️ Add all channels for complete stats</small>' : ''}</td>
...
async function saveExperimentStatistics(experimentPattern, allResults, fluorophores) {
    try {
        console.log('Saving experiment statistics:', experimentPattern);
        ...
        if (response.ok) {
            const result = await response.json();
            console.log('Experiment statistics saved:', result.message);
        } else {
...
async function viewTrendAnalysis() {
    try {
        ...
        if (!data.experiments || data.experiments.length === 0) {
            alert('No trend analysis data available. Complete some analyses first to build trend history.');
            return;
        }
...
function displayTrendAnalysis(experiments) {
    ...
    if (!historyContent) {
        ...
        throw new Error('Cannot display trend analysis - history container not found');
    }
    ...
    experiments.forEach(exp => {
        ...
        const stats = exp.fluorophore_stats || {};
        try {
            ...
            console.error('Error processing fluorophore stats:', error, 'Stats:', stats);
            throw new Error(`Failed to process trend data: ${error.message}`);
        }
    });
    ...
    Object.entries(testSummary).forEach(([testName, summary]) => {
        ...
        Object.entries(summary.fluorophores).forEach(([fluorophore, fluorStats]) => {
            ...
            try {
                pathogenTarget = typeof getPathogenTarget === 'function' ? getPathogenTarget(testCode, fluorophore) : 'Unknown';
            } catch (e) {
                console.warn('getPathogenTarget function not available:', e);
            }
            
            try {
                fluorophoreColor = typeof getFluorophoreColor === 'function' ? getFluorophoreColor(fluorophore) : '#666';
            } catch (e) {
                console.warn('getFluorophoreColor function not available:', e);
            }
...
async function displaySessionResults(session) {
    ...
    try {
        ...
        // Debug: log each session's data
        sessionDataArray.forEach((sessionData, index) => {
            console.log(`Session ${index + 1}:`, {
                sessionName: sessionData.session?.filename,
        ...
        // Combine all wells from all sessions
        const allWells = [];
        let totalWells = 0;
        let totalPositive = 0;
        ...
        sessionDataArray.forEach((sessionData, sessionIndex) => {
            if (sessionData.wells) {
                console.log(`Adding ${sessionData.wells.length} wells from session ${sessionIndex + 1}`);
                allWells.push(...sessionData.wells);
                totalWells += sessionData.wells.length;
        ...
        allWells.forEach((well, index) => {
            const fluorophore = well.fluorophore || extractFluorophoreFromWellId(well.well_id) || 'Unknown';
            const baseWellId = well.well_id.includes('_') ? well.well_id.split('_')[0] : well.well_id;
            
            // Create unique well key with fluorophore suffix if not already present
            const wellKey = well.well_id.includes('_') ? well.well_id : `${well.well_id}_${fluorophore}`;
            
            // Debug well key generation for first few wells
            if (index < 5) {
                console.log(`Well ${index}: original=${well.well_id}, fluorophore=${fluorophore}, key=${wellKey}`);
            }
