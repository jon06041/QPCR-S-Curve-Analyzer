...
function displayPathogenChannelStatusInBreakdown(testStatus) {
    ...
    if (statusHtml) {
        ...
        `;
    } else if (hasCurrentExperiment || hasAnyLoadedSession) {
        // Check if we're looking at a loaded session vs fresh analysis
        const isLoadedSession = !amplificationFiles || Object.keys(amplificationFiles).length === 0;
        
        if (isLoadedSession && hasAnyLoadedSession) {
            // For loaded sessions, always show complete status since they were previously completed
            statusContainer.innerHTML = '<div class="all-complete">✓ All pathogen channels complete</div>';
        } else {
            // For fresh analysis, check actual completion status
            const anyComplete = Object.values(testStatus).some(test => 
                test.experiments.some(exp => 
                    (currentPattern && exp.experimentPattern === currentPattern && exp.validation && exp.validation.isComplete) ||
                    (hasAnyLoadedSession && exp.validation && exp.validation.isComplete)
                )
            );
...
function getHistoryValidationMessage(session) {
    // Skip validation message for combined sessions
    ...
    // Default for multi-channel tests without validation
    return '<br><small style="color: #e67e22;">⚠️ Add all channels for complete stats</small>';
}
...
function displayAnalysisHistory(sessions) {
    ...
    const tableHtml = `
        <div class="history-table-header" style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 10px;">
            <span style="font-weight: bold; color: #2c3e50;">Analysis Sessions</span>
            <button class="control-btn delete-all-btn" onclick="deleteAllSessions()" style="background: #e74c3c; color: white; font-size: 0.9rem;">Delete All</button>
        </div>
...
async function displaySessionResults(session) {
    ...
    try {
        ...
        setTimeout(() => {
            ...
            updateExportButton(false, []);
            // Show complete status for loaded sessions
            const statusContainer = document.getElementById('pathogenChannelStatus');
            if (statusContainer) {
                statusContainer.innerHTML = '<div class="all-complete">✓ All pathogen channels complete</div>';
            }
