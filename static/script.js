...
function showWellDetails(wellKey) {
    ...
    const shouldShowFilteredSamples = (effectiveFilterMode === 'pos' || effectiveFilterMode === 'redo') && 
...
function generateFilteredSamplesHtml(effectiveFilterMode = null) {
    ...
    const filterType = filterMode === 'pos' ? 'POS' : 'REDO';
...
async function loadSessionDetails(sessionId) {
    try {
        ...
        // Reset filter states when loading historical sessions
...
function addFluorophoreFilter(individualResults) {
    ...
    if (fluorophores.length <= 1) {
        // Remove existing filter if only one fluorophore
        const existingFilter = document.querySelector('#fluorophoreFilterRow');
        if (existingFilter) {
            existingFilter.remove();
        }
...
function displayTrendAnalysis(experiments) {
    ...
    // Group experiments by test name and aggregate statistics
    const testSummary = {};
    ...
    Object.entries(testSummary).forEach(([testName, summary]) => {
        ...
        html += `<span class="test-info">Latest Analysis: ${summary.latestTimestamp.toLocaleString()}</span>`;
        html += `</div>`;
        ...
        Object.entries(summary.fluorophores).forEach(([fluorophore, fluorStats]) => {
            ...
            html += `<div class="fluorophore-trend-label">${fluorophore} Subtest:</div>`;
...
function displayFluorophoreBreakdown(fluorophoreStats) {
    ...
    let breakdownHTML = '<h4>Fluorophore Breakdown</h4><div class="fluorophore-stats">';
    ...
    Object.entries(fluorophoreStats).forEach(([fluorophore, stats]) => {
        ...
        breakdownHTML += `
            <div class="fluorophore-stat-card">
                <div class="fluorophore-name">${fluorophore}</div>
...
function showWellModal(wellKey) {
    ...
    const wellId = wellResult.well_id || wellKey.split('_')[0];
    const fluorophore = wellResult.fluorophore || 'Unknown';
    const experimentPattern = getCurrentFullPattern();
    ...
    populateModalDetails(wellKey, wellResult);
}
