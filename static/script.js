...
function updateExportButton(hasIncompleteTests, incompleteTestsInfo) {
    ...
    if (exportButton) {
        ...
        // Always enable export if we have analysis results - don't block on channel requirements for loaded sessions
        if (hasAnalysisResults) {
            exportButton.disabled = false;
            exportButton.style.opacity = '1';
            exportButton.style.cursor = 'pointer';
        ...
        if (testCode && getRequiredChannels) {
            ...
            if (statusContainer) {
                if (validation.isComplete) {
                    // Show complete status for all complete tests (single or multi-channel)
