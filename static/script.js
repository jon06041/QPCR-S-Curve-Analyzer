...
function updateExportButton(hasIncompleteTests, incompleteTestsInfo) {
    ...
    if (exportButton) {
        ...
        if (isCurrentExperimentComplete) {
            ...
            exportButton.title = 'Complete current experiment analysis first to enable export';
...
function checkCurrentExperimentComplete() {
    ...
    // For multi-fluorophore sessions, check if we have all three channels
    ...
    // Complete if we have 3 fluorophores (Cy5, FAM, HEX) or it's a single channel session
    return fluorophores.size >= 3 || Object.keys(results).length > 0;
}
...
async function displaySessionResults(session) {
    ...
    try {
        ...
        // Save experiment statistics for trend analysis if this is a complete multi-fluorophore session
        if (constituentSessions.length >= 3) {
            const experimentPattern = extractBasePattern(session.filename);
            const fluorophores = ['Cy5', 'FAM', 'HEX'];
            setTimeout(() => {
                saveExperimentStatistics(experimentPattern, transformedResults, fluorophores);
            }, 500);
        }
