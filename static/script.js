...
function updateExportButton(hasIncompleteTests, incompleteTestsInfo) {
    ...
    if (exportButton) {
        ...
        // Get current experiment pattern and check completion for THIS specific experiment
        const currentPattern = getCurrentFullPattern();
        if (!currentPattern) {
        ...
        // Extract fluorophores from current loaded results
        ...
        // Also check fluorophore field in results for single-channel tests
        ...
        // For individual channel sessions, detect fluorophore from filename with enhanced detection
        ...
        console.log('Export validation:', {
            ...
            wellKeys: Object.keys(currentAnalysisResults.individual_results).slice(0, 5)
        });
        ...
        if (hasAllRequiredChannels) {
            ...
            exportButton.style.opacity = '0.5';
            exportButton.style.cursor = 'not-allowed';
            exportButton.title = `Export disabled: ${testCode} requires ${requiredChannels.join(', ')} channels. Missing: ${missingChannels.join(', ')}`;
            exportButton.textContent = 'Export Disabled';
            console.log(`Export DISABLED: ${testCode} missing channels ${missingChannels.join(', ')}`);
        }
    }
}
...
async function updatePathogenChannelStatusInBreakdown() {
    try {
        ...
        // Skip if this is a loaded historical session (has session_id)
        if (currentAnalysisResults.session_id) {
            return;
        }
        ...
        if (testCode && getRequiredChannels) {
            ...
            if (availableChannels.size === 0 && currentAnalysisResults.filename) {
                const detectedFluorophore = detectFluorophoreFromFilename(currentAnalysisResults.filename);
                if (detectedFluorophore && detectedFluorophore !== 'Unknown') {
                    availableChannels.add(detectedFluorophore);
                }
            }
            ...
            // Only show warning if incomplete - never show "complete" for single channels
            const statusContainer = document.getElementById('pathogenChannelStatus');
            if (statusContainer && !validation.isComplete) {
                ...
                `;
                statusContainer.style.display = 'block';
            } else if (statusContainer && validation.isComplete) {
                // Only show complete status for truly complete experiments (3 channels)
...
function calculatePositiveRate(session) {
    ...
    // Check if stored pathogen breakdown contains "Unknown" - if so, recalculate
    if (session.pathogen_breakdown && !session.pathogen_breakdown.includes('Unknown')) {
    ...
    console.log('🔍 HISTORY DEBUG - Stored breakdown contains Unknown or missing, recalculating...');
...
async function viewTrendAnalysis() {
    try {
        ...
        // Filter experiments based on their pathogen library requirements
        let validExperiments = [];
        if (data && data.experiments) {
            validExperiments = data.experiments.filter(exp => {
                const testCode = exp.test_name;
                if (!testCode || !getRequiredChannels) return false;
                ...
                // Check if experiment has all required channels with valid data
                const hasAllRequiredChannels = requiredChannels.every(channel => 
                    breakdown[channel] && breakdown[channel].total > 0
                );
        ...
        if (validExperiments.length === 0) {
            console.warn('No valid complete experiments for trend analysis');
            ...
            if (historyContent) {
                historyContent.innerHTML = `
                    <div class="trend-analysis-section">
                        ...
                        <p>No complete experiments available for trend analysis. Complete a full 3-channel analysis first.</p>
