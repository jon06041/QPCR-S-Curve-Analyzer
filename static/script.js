...
function updateExportButton(hasIncompleteTests, incompleteTestsInfo) {
    ...
    if (exportButton) {
        // Simple rule: Enable export if ANY analysis results are loaded
        // This allows exporting historical sessions and partial current analysis
...
async function viewTrendAnalysis() {
    try {
        ...
        // Filter out any incomplete experiments that may have been saved incorrectly
        let validExperiments = [];
        if (data && data.experiments) {
            validExperiments = data.experiments.filter(exp => {
                // Ensure experiment has all required fluorophores
                const breakdown = exp.fluorophore_stats;
                const hasAllChannels = breakdown && 
                                     breakdown.Cy5 && 
                                     breakdown.FAM && 
                                     breakdown.HEX;
                
                // Ensure each channel has meaningful data
                const hasValidData = hasAllChannels &&
                                   breakdown.Cy5.total > 0 &&
                                   breakdown.FAM.total > 0 &&
                                   breakdown.HEX.total > 0;
...
async function ensureAllExperimentStatistics() {
    try {
        ...
        // Check each experiment group for completeness - STRICT validation
        ...
        Object.entries(experimentGroups).forEach(([pattern, groupSessions]) => {
            ...
            // STRICT: BVAB requires exactly 3 specific channels AND valid data in all sessions
            if (hasValidData && 
                fluorophores.size === 3 && 
                fluorophores.has('Cy5') && 
                fluorophores.has('FAM') && 
                fluorophores.has('HEX')) {
